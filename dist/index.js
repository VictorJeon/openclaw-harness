...
function processReviewResult(state, reviewResult) {
...
function buildFixPrompt(review, loopNumber) {
  const lines = [
...
function formatEscalation(plan, state, task) {
  const lines = [
    `## Harness Escalation`,
    ``,
      ...
      if (action.action === "fix") {
        if (useBackendDispatch && backend) {
          ...
          if (!isRealtimeReviewReadyStatus(followUpResult.status) || !followUpResult.workerResult) {
            ...
            const followUpError = followUpResult.errorDetail ?? followUpResult.error ?? `Worker follow-up ended with status=${followUpResult.status}`;
            return {
              taskId: task.id,
              workerSessionId,
              workerResult: followUpResult.workerResult ?? currentWorkerResult,
